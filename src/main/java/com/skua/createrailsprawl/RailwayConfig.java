...
public class RailwayConfig {
    ...
    private static final ForgeConfigSpec.DoubleValue HEURISTIC_WEIGHT = BUILDER
            ...
            .defineInRange("heuristicWeight", 1.2, 0.1, 5.0);

    private static final ForgeConfigSpec.DoubleValue DEVIATION_WEIGHT = BUILDER
            .comment("Weight for path deviation from straight line (higher = straighter paths)")
            .defineInRange("deviationWeight", 0.3, 0.0, 2.0);

    private static final ForgeConfigSpec.DoubleValue BIOME_WEIGHT = BUILDER
            .comment("Weight for biome-based costs (water, mountains)")
            .defineInRange("biomeWeight", 1.0, 0.0, 5.0);

    private static final ForgeConfigSpec.DoubleValue STABILITY_WEIGHT = BUILDER
            .comment("Weight for terrain stability")
            .defineInRange("stabilityWeight", 0.5, 0.0, 3.0);

    private static final ForgeConfigSpec.DoubleValue FLOATING_WEIGHT = BUILDER
            .comment("Weight for bridge/floating sections")
            .defineInRange("floatingWeight", 1.5, 0.0, 5.0);

    private static final ForgeConfigSpec.DoubleValue PENETRATION_WEIGHT = BUILDER
            .comment("Weight for tunnel/penetration sections")
            .defineInRange("penetrationWeight", 2.0, 0.0, 5.0);

    private static final ForgeConfigSpec.IntValue BRIDGE_MIN_WATER_DEPTH = BUILDER
            .comment("Minimum water depth to trigger bridge mode")
            .defineInRange("bridgeMinWaterDepth", 3, 1, 20);

    private static final ForgeConfigSpec.IntValue ASTAR_STEP = BUILDER
            .comment("A* pathfinding step size")
            .defineInRange("aStarStep", 8, 1, 32);

    private static final ForgeConfigSpec.DoubleValue ORTHO_STEP_COST = BUILDER
            .comment("Cost for orthogonal movement")
            .defineInRange("orthoStepCost", 1.0, 0.1, 5.0);

    private static final ForgeConfigSpec.DoubleValue DIAG_STEP_COST = BUILDER
            .comment("Cost for diagonal movement")
            .defineInRange("diagStepCost", 1.414, 0.1, 5.0);
    ...
    public static double deviationWeight;
    public static double biomeWeight;
    public static double stabilityWeight;
    public static double floatingWeight;
    public static double penetrationWeight;
    public static int bridgeMinWaterDepth;
    public static int aStarStep;
    public static double orthoStepCost;
    public static double diagStepCost;
    ...
    static void onLoad(final ModConfigEvent event) {
        ...
        deviationWeight = DEVIATION_WEIGHT.get();
        biomeWeight = BIOME_WEIGHT.get();
        stabilityWeight = STABILITY_WEIGHT.get();
        floatingWeight = FLOATING_WEIGHT.get();
        penetrationWeight = PENETRATION_WEIGHT.get();
        bridgeMinWaterDepth = BRIDGE_MIN_WATER_DEPTH.get();
        aStarStep = ASTAR_STEP.get();
        orthoStepCost = ORTHO_STEP_COST.get();
        diagStepCost = DIAG_STEP_COST.get();
    ...
    private static void sanitize() {
        ...
        if (deviationWeight < 0.0) deviationWeight = 0.0;
        if (deviationWeight > 2.0) deviationWeight = 2.0;

        if (biomeWeight < 0.0) biomeWeight = 0.0;
        if (biomeWeight > 5.0) biomeWeight = 5.0;

        if (stabilityWeight < 0.0) stabilityWeight = 0.0;
        if (stabilityWeight > 3.0) stabilityWeight = 3.0;

        if (floatingWeight < 0.0) floatingWeight = 0.0;
        if (floatingWeight > 5.0) floatingWeight = 5.0;

        if (penetrationWeight < 0.0) penetrationWeight = 0.0;
        if (penetrationWeight > 5.0) penetrationWeight = 5.0;

        if (bridgeMinWaterDepth < 1) bridgeMinWaterDepth = 1;
        if (bridgeMinWaterDepth > 20) bridgeMinWaterDepth = 20;

        if (aStarStep < 1) aStarStep = 1;
        if (aStarStep > 32) aStarStep = 32;

        if (orthoStepCost < 0.1) orthoStepCost = 0.1;
        if (orthoStepCost > 5.0) orthoStepCost = 5.0;

        if (diagStepCost < 0.1) diagStepCost = 0.1;
        if (diagStepCost > 5.0) diagStepCost = 5.0;
    }
